...
import {
  showapiError, shownoResults, callTosearch, shownoMessage,
...
const Home = () => {
  ...
  return (
    <div style={{ backgroundColor: mode === 'browser' ? 'white' : mode, padding: '2.5% 0' }}>
      <Alert
        variant={mode === 'black' ? 'light' : 'dark'}
        style={{
          textAlign: 'center', width: '60%', marginLeft: '20%', fontFamily: 'serif', fontWeight: '550', display: (keyWord.trim().length > 0 && tvShows.length > 0) ? 'none' : 'block',
        }}
      >
        {message}
      </Alert>
